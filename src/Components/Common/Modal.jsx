...
const Modal = ({ showHistory, showGuide, wrongGuesses, data }) => {
  ...
  const renderNotActiveItem = ({ item }) => (
    <View className="mb-5 text-center">
      <View className="flex-row justify-center items-center gap-2">
        ...
        <Text className="text-2xl">{item.word.charAt(0).toUpperCase() + item.word.slice(1)}</Text>
        <View className="w-full h-px bg-black " />
      </View>
  ...
  return (
      ...
      <View className="p-6 bg-white rounded-lg shadow-lg justify-between items-center w-4/5">
        ...
        <Button style="normal" title="Sulge" onPress={() => console.log("sulge pressed")} />
      </View>
